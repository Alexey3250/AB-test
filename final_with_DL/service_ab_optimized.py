...
'''
Получение фичей из базы данных
'''
def load_features(user_id: int) -> pd.DataFrame:
    # Query to load features only for the specific user_id
    query = f"SELECT * FROM alexey_efimik_data_ready_wv3 WHERE user_id = {user_id}"
    engine = create_engine(
        "postgresql://robot-startml-ro:pheiph0hahj1Vaif@"
        "postgres.lab.karpov.courses:6432/startml"
    )
